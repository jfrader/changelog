...
import { entryFileName, slugify, parseEntry, ENTRY_PATTERN } from './entry.js';
...
import { KIND_ORDER, type ChangelogConfig, type ChangelogKind } from './types.js';
import {
  ENTRY_TEMPLATE,
  README_TEMPLATE,
  RUNBOOK_TEMPLATE,
...
export async function init(options: InitOptions): Promise<string> {
  ...
  if (!options.force) {
    ...
    if (existingConfig.product !== DEFAULT_CONFIG.product || (await fs.readdir(changelogDir)).length > 0) {
      ...
      if (hasConfig) {
        throw new Error(
          `A changelog already exists at ${changelogDir}. Re-run with --force to overwrite config and re-create templates.`,
        );
      }
    }
  }
  ...
  const fill = (template: string) =>
    template.replace(/PRODUCT_NAME/g, config.productName).replace(/PRODUCT/g, config.product);
...
export async function add(options: AddOptions): Promise<string> {
  ...
  if (!project) {
    throw new Error(
      `No changelog found at or above ${path.resolve(options.root)}. Run \`changelog init\` first.`,
    );
  }
...
export async function listEntries(options: ListOptions): Promise<string[]> {
  const project = await findProject(options.root);
  if (!project) return [];
  const entriesDir = await entryDirOf(project);
  let files: string[] = [];
  ...
  if (options.all) return matching;
  ...
  for (const file of matching) {
    try {
      ...
      if (parsed.frontmatter['published'] !== false) published.push(file);
    } catch {
...
export async function build(options: { root: string }): Promise<{ outputs: string[]; warnings: string[]; errors: string[]; counts: { entries: number; published: number } }> {
  const project = await findProject(options.root);
  if (!project) {
...
export async function serve(options: ServeOptions): Promise<{ port: number; close: () => Promise<void> }> {
  const project = await findProject(options.root);
  if (!project) {
    throw new Error(`No changelog found at or above ${path.resolve(options.root)}. Run \`changelog init\` first.`);
  }
